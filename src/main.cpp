...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // rurc: starting difficulty is 0.0002441
...
const string strMessageMagic = "rurc Signed Message:\n";
...
{
    // rurc: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 50 * COIN; // 50 RURC block reward
	
	// Subsidy is cut in half every 210240 blocks, which will occur approximately every year
    nSubsidy >>= (nHeight / 210240); // Rurc: 210240 blocks in ~one year
...
static const int64 nTargetTimespan = 10 * 30; // rurc: 1 day difficulty retarget time
static const int64 nTargetSpacing = 1 * 30; // rurc: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // rurc: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        int DiffMode = 2; // KGW
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 2116) { DiffMode = 2; } // rurc, 100 blocks after first legacy diff adjustment
        }
        else {
        	if (pindexLast->nHeight+1 >= 26754) { DiffMode = 2; }  //rurc, 5 days after 27/01/2014 12:00 UTC
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // rurc: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // rurc: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
		...
		BOOST_FOREACH(const CTransaction& tx, pblock->vtx){
			...
			for(unsigned int i = 0; i < vtxOut.size(); i++){
				...
				if(txOut.scriptPubKey[0] == OP_RETURN && txOut.scriptPubKey[1] == 0x21){
					...
					// generate rurc address from ephem_pubkey, scan_secret and spend_secret
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0"); // Testnet genesis block hash
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "13 08 2015 Left Handers Day";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce = 0;
        block.nTime = 1439459675; // Mainnet epoch

        if (fTestNet)
        {

            block.nNonce = 0;
            block.nTime = 1439459674; // Testnet epoch
        }
...
unsigned char pchMessageStart[4] = { 0x7d, 0x7d, 0x7d, 0x7d }; // rurc: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("rurcMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("rurcMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static rurcMiner(CWallet *pwallet)
{
    printf("rurcMiner started\n");

    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rurc-miner");
    ...
    try { loop {
        ...
        printf("Running rurcMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&rurcMiner, pwallet));
}
