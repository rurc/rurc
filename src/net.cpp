...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "rurc " + FormatFullVersion();
...
unsigned int pnSeed[] =
{
//    0x00000000, // rurc hardcoded seed
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. rurc is probably already running."), addrBind.ToString().c_str());
        else
